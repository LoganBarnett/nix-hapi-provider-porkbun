# NixOS module for the nix-hapi-provider-porkbun-web service.
# Exported from the flake as nixosModules.web.
#
# Minimal usage (defaults to Unix domain socket):
#
#   inputs.nix-hapi-provider-porkbun.nixosModules.web
#
#   services.nix-hapi-provider-porkbun-web = {
#     enable = true;
#   };
#
# To use TCP instead:
...
#
#   locations."/".proxyPass =
#     "http://unix:${config.services.nix-hapi-provider-porkbun-web.socket}";
#
...
{self}: {
  ...
  cfg = config.services.nix-hapi-provider-porkbun-web;
in {
  options.services.nix-hapi-provider-porkbun-web = {
    enable = lib.mkEnableOption "nix-hapi-provider-porkbun-web web service";
    ...
    socket = lib.mkOption {
      ...
      default = "/run/nix-hapi-provider-porkbun-web/nix-hapi-provider-porkbun-web.sock";
      description = ''
    ...
    frontendPath = lib.mkOption {
      type = lib.types.str;
      default = "${cfg.package}/share/nix-hapi-provider-porkbun-web/frontend";
      defaultText =
        lib.literalExpression
        ''"''${cfg.package}/share/nix-hapi-provider-porkbun-web/frontend"'';
    ...
    user = lib.mkOption {
      type = lib.types.str;
      default = "nix-hapi-provider-porkbun-web";
  ...
  config = lib.mkIf cfg.enable {
    users.users.${cfg.user} = {
      isSystemUser = true;
      group = cfg.group;
      description = "nix-hapi-provider-porkbun-web service user";
    };
    ...
    systemd.sockets.nix-hapi-provider-porkbun-web = lib.mkIf (cfg.socket != null) {
      description = "nix-hapi-provider-porkbun-web Unix domain socket";
    ...
    systemd.services.nix-hapi-provider-porkbun-web = {
      description = "nix-hapi-provider-porkbun-web web service";
      wantedBy = ["multi-user.target"];
      after =
        ["network.target"]
        ++ lib.optional (cfg.socket != null) "nix-hapi-provider-porkbun-web.socket";
      requires =
        lib.optional (cfg.socket != null) "nix-hapi-provider-porkbun-web.socket";
      ...
      serviceConfig = {
        ...
        # Override via systemd.services.nix-hapi-provider-porkbun-web.serviceConfig.WatchdogSec.
        WatchdogSec = lib.mkDefault "30s";

        ExecStart =
          "${cfg.package}/bin/nix-hapi-provider-porkbun-web"
          + (
            if cfg.socket != null
