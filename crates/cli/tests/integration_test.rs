...
fn get_binary_path() -> PathBuf {
  ...
  path.push("nix-hapi-provider-porkbun-cli");
  ...
  match output {
    ...
    Err(e) => {
      if e.kind() == std::io::ErrorKind::NotFound {
        eprintln!(
                    "Binary not found. Please build the project first with: cargo build -p nix-hapi-provider-porkbun-cli"
                );
      }
...
fn test_version_flag() {
  ...
  match output {
    Ok(output) => {
      ...
      let stdout = String::from_utf8_lossy(&output.stdout);
      assert!(
        stdout.contains("nix-hapi-provider-porkbun-cli"),
        "Expected version text to contain 'nix-hapi-provider-porkbun-cli', got: {}",
        stdout
      );
    }
    Err(e) => {
