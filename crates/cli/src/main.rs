//! nix-hapi-provider-porkbun-cli - Command-line application template
//!
...
fn main() -> Result<(), ApplicationError> {
  ...
  info!("Starting nix-hapi-provider-porkbun-cli");
  info!("Configuration loaded successfully");

  run(config)?;

  info!("Shutting down nix-hapi-provider-porkbun-cli");
  Ok(())
}
